...
json_file = "/home/shaurya/Pokemon_HACK/test_images/roberta_pretrained_zeroshot.json"      # mapping file
output_csv = "pokemon_pretrained_roberta_more_scenes_0.4_0.85_0.9_avgrefined.csv"
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if cls_name.lower() == target_pokemon.lower():
                ...
                # --- Collect centers from passes ---
                centers_pass = []

                # First pass center
                cx1 = round((x1 + x2) / 2, 2)
                cy1 = round((y1 + y2) / 2, 2)
                centers_pass.append([cx1, cy1])

                # Crop region of interest for second pass
                ...
                for sr in second_results:
                    for sbox in sr.boxes:
                        ...
                        if s_cls_name.lower() == target_pokemon.lower():
                            ...
                            cx2 = round((sx1 + sx2) / 2, 2)
                            cy2 = round((sy1 + sy2) / 2, 2)
                            centers_pass.append([cx2, cy2])
                            ...
                            for tr in third_results:
                                for tbox in tr.boxes:
                                    ...
                                    if t_cls_name.lower() == target_pokemon.lower():
                                        ...
                                        cx3 = round((tx1 + tx2) / 2, 2)
                                        cy3 = round((ty1 + ty2) / 2, 2)
                                        centers_pass.append([cx3, cy3])

                # --- Final averaged center ---
                if centers_pass:
                    avg_cx = round(sum(c[0] for c in centers_pass) / len(centers_pass), 2)
                    avg_cy = round(sum(c[1] for c in centers_pass) / len(centers_pass), 2)
                    refined_center = [avg_cx, avg_cy]
                else:
                    refined_center = [cx1, cy1]  # fallback first pass
